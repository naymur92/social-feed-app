...
export default function CreatePost({ onCreated }) {
  ...
  function handleFiles(e) {
    const picked = [...e.target.files].slice(0, MAX_IMAGES);
    setFiles(picked);
  ...
  async function submit() {
    ...
    try {
      const res = await api("/posts", { method: "POST", body: fd, token, isFormData: true });
      onCreated(res.data);
      setContent("");
      setFiles([]);
      setPreviews([]);
    ...
    <div className="_feed_inner_text_area  _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
      <div className="_feed_inner_text_area_box">
        <div className="_feed_inner_text_area_box_image">
          <Avatar src={user?.profile_picture} name={user?.full_name || ""} size={40} className="_txt_img" />
        </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "12px 0" }}>
          {previews.map((p, i) => (
            <div key={i} style={{ position: "relative" }}>
              <img src={p} alt="preview" style={{ width: 72, height: 72, objectFit: "cover", borderRadius: 6 }} />
              <button type="button" onClick={() => removeFile(i)} aria-label="Remove image"
                style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: "50%", border: "none", background: "#00000099", color: "#fff", fontSize: 11, lineHeight: "20px", cursor: "pointer" }}>
                ✕
              </button>
            </div>
          ))}
      ...
      <div className="_feed_inner_text_area_bottom">
        <div className="_feed_inner_text_area_item">
          <div className="_feed_inner_text_area_bottom_photo _feed_common">
            <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => fileRef.current?.click()}> <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
        ...
        <div className="_feed_inner_text_area_btn" style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <select
            value={visibility}
          ...
          <button type="button" className="_feed_inner_text_area_btn_link" onClick={submit} disabled={submitting}>
