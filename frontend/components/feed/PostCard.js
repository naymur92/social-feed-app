...
export default function PostCard({ post }) {
  ...
  async function toggleLike() {
    ...
    try {
      const res = await api(`/posts/${post.id}/like`, { method: "POST", token });
  ...
  return (
    <>
      <div className="_feed_inner_timeline_post_area _b_radious6 _padd_b24 _padd_t24 _mar_b16">
          ...
          <div className="_feed_inner_timeline_post_top">
            <div className="_feed_inner_timeline_post_box">
              <div className="_feed_inner_timeline_post_box_image">
                <Avatar src={post.user.profile_picture} name={post.user.full_name} size={44} className="_post_img" />
              </div>
              <div className="_feed_inner_timeline_post_box_txt">
                ...
                <p className="_feed_inner_timeline_post_box_para">{post.created_at} .
                  <a href="#0" onClick={(e) => e.preventDefault()}>{post.visibility ? post.visibility.charAt(0).toUpperCase() + post.visibility.slice(1) : "Public"}</a>
                </p>
              </div>
            </div>
            <div className="_feed_inner_timeline_post_box_dropdown">
              <div className="_feed_timeline_post_dropdown">
                <button type="button" className="_feed_timeline_post_dropdown_link" onClick={() => setMenuOpen(!menuOpen)}>
              ...
              {/* dropdown */}
              <div className="_feed_timeline_dropdown _timeline_dropdown" style={menuOpen ? { display: "block", visibility: "visible", opacity: 1, transform: "none" } : undefined}>
          ...
          {post.images?.length > 0 && (
            <div className="_feed_inner_timeline_image">
              {post.images.map((img, i) => (
                <img key={i} src={typeof img === "string" ? img : img.url} alt="" className="_time_img" />
              ))}
            </div>
          )}
        </div>
        <div className="_feed_inner_timeline_total_reacts _padd_r24 _padd_l24 _mar_b26">
          <div className="_feed_inner_timeline_total_reacts_image" onClick={() => likesCount > 0 && setShowLikers(true)} role="button" style={{ cursor: likesCount > 0 ? "pointer" : "default" }}>
            <p className="_feed_inner_timeline_total_reacts_para">{likesCount}</p>
          </div>
          <div className="_feed_inner_timeline_total_reacts_txt">
            <p className="_feed_inner_timeline_total_reacts_para1">
              <a href="#0" onClick={(e) => e.preventDefault()}><span>{commentsCount}</span> Comment</a>
            </p>
            <p className="_feed_inner_timeline_total_reacts_para2"><span>0</span> Share</p>
          </div>
        </div>
        <div className="_feed_inner_timeline_reaction">
          <button type="button" className={`_feed_inner_timeline_reaction_emoji _feed_reaction${liked ? " _feed_reaction_active" : ""}`} onClick={toggleLike} disabled={busy}>
        ...
        <CommentsSection post={post} onCountChange={(d) => setCommentsCount((c) => c + d)} />
      </div>
      {showLikers && (
        <LikersModal type="posts" id={post.id} onClose={() => setShowLikers(false)} />
      )}
    </>
  );
}