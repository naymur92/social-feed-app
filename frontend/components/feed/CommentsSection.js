...
export default function CommentsSection({ post, onCountChange }) {
  ...
  const loadComments = useCallback(
    async (cursor = null) => {
      ...
      try {
        const res = await api(
          `/posts/${post.id}/comments${cursor ? `?cursor=${encodeURIComponent(cursor)}` : ""}`,
          { token }
        );

        const batch = [...res.data].reverse();
        setComments((prev) => (cursor ? [...batch, ...prev] : batch));
  ...
  useEffect(() => {
    if (post.comments_count > 0) loadComments();
  }, [post.comments_count, loadComments]);
    ...
    <div className="_timline_comment_main">
      ...
      {hasMore && (
        <div className="_previous_comment">
          <button type="button" className="_previous_comment_txt" onClick={() => loadComments(nextCursor)} disabled={loading}>
      ...
      {comments.map((c) => (
        <CommentItem
          ...
          onReplyCreated={(parentId, reply) => {
            setComments((prev) =>
              prev.map((x) =>
                x.id === parentId ? { ...x, replies: [...(x.replies ?? []), reply] } : x
              )
            );
            onCountChange?.(1);
          }}
